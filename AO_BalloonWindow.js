...
オブジェクトの台詞を個別にまたは全てクリアするプラグインコマンド？←オブジェクトに対するコマンドはまだ！突然表示が消えることを考えたら実装できないかも！
アクションシーケンス等や戦闘不能・攻撃時に自動で台詞を表示する機能？台詞の読み込みはどこのテキストを読み込むの？

既存文章表示コマンドの一時的のっとりプラグインコマンドを作製←できたかな？不具合確認中
スプライト四辺にオフセットを設定できるメモ欄の記載かプラグインコマンド←メモ欄はOK！コマンドはまだ

自身の表示時間を設定する制御文字を！
キューを個別にまたは全てクリアするプラグインコマンド
フロントビュー時の仕様決定←スプライト位置をプラグインパラメータで指定する？四人目以降は知らんが
キューで次の文章までの時間を指定できるコマンドか制御文字を！←できたかな？
シーンマップキューとシーンバトルキューの分離。シーン遷移時のキューのアップデートの休止
SE再生機能を追加
制御文字を整備
アンカーの形とウインドウの形を別々に制御できるようにしようか？
初期位置を制御文字で決定できるように
テキストアラインの制御文字は？
キューを実装。できれば個数制限無しで。
当たり判定と動きを実装したいが･･･
